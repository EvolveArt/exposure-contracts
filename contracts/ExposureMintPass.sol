    ...
    ERC1155Burnable
{
    ...
    event Claimed(uint256 mpId, address indexed account, uint256 amount);
    ...
    function setContractURI(string calldata newUri) public override onlyOwner {
        super.setContractURI(newUri);
    }

    function setRoyalteFee(uint256 fee) public override onlyOwner {
        royaltyFee = fee;
    }
    ...
    function burnFromRedeem(
        ...
        require(
            mintPasses[mpId].redeemableContract == msg.sender,
            "burnFromRedeem: Only allowed from redeemable contract"
        );
    ...
    function claim(
        ...
        require(
            MerkleProof.verify(merkleProof, mp.merkleRoot, node),
            "claim: Invalid proof."
        );
    ...
    function uri(uint256 _id) public view override returns (string memory) {
        ...
        return
            string(abi.encodePacked(super.uri(_id), mintPasses[_id].uriSuffix));
    }
